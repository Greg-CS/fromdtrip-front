...
export default function ProductPage({ product, newProducts, CategoryProp }) {
  ...
  return (
    <>
      <div className="bg-[#07100B] p-12 text-[#71948D]">
        <div className=" lg:flex lg:justify-between p-2 rounded-2xl lg:p-12 gap-4 bg-[#354A4B] lg:rounded-2xl">
          ...
          <div className="w-full lg:w-4/12">
            <div className="grid items-center gap-5 pb-5 lg:flex">
              ...
              <span
                className="p-4 border-2 border-transparent text-[#71948D] text-2xl font-bold badge bg-[#1F3C2A] badge-neutral badge-lg"
            ...
            <div className="lg:grid">
              ...
              <div className="flex justify-end">
                <button
                  className="ui-btn flex gap-4 disabled:bg-[#71948D] disabled:text-slate-300"
                  onClick={() => {
              ...
              {CategoryProp.properties.map((Cat) => {
                return (
                  <div key={Cat.name} className="py-3">
                    <label className="text-xl">{Cat.name}</label>
                    <div className="grid grid-cols-3 gap-10 pt-1 lg:grid-cols-4 justify-evenly">
                      {Cat.values.map((v) => {
                        return (
                          <div key={v}>
                            <button
                              className={`bg-[#1F3C2A] h-auto w-16 p-2 rounded-2xl text-md hover:bg-[#71948D] hover:text-[#73848C] ${
                                selectedButtons[Cat.name] === v
                                  ? "bg-[#5A6B58] text-[#71948D]"
                                  : ""
                              }`}
